...
class RetryableExceptionTest {
    ...
    fun non_throwing_function_works() {
        assertThat(RetryableException.retryOnce { called++ }, equalTo(0))
    ...
    fun conditionally_throwing_function_retries() {
        assertThat(
            RetryableException.retryOnce { called++.also { if (called == 1) { throw RetryableException(TestException("throwing_function_retries")) } } },
            equalTo(1)
        )
