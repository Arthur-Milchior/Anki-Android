...
class UndoTest : RobolectricTest() {
    ...
    fun test_op() {
        ...
        with(col) {
            ...
            */
            ...
            sched.answerCard(col, c!!, Consts.BUTTON_TWO)
    ...
    fun test_review() {
        ...
        with(col) {
            ...
            */
            // answer
            assertEquals(Counts(1, 0, 0), sched.counts(col))
            var c = sched.card(col)
            assertEquals(QUEUE_TYPE_NEW, c!!.queue)
            sched.answerCard(col, c, Consts.BUTTON_THREE)
            assertEquals(1001, c.left)
            assertEquals(Counts(0, 1, 0), sched.counts(col))
            ...
            addNote(note)
            reset()
            assertEquals(Counts(2, 0, 0), sched.counts(col))
            c = sched.card(col)
            sched.answerCard(col, c!!, Consts.BUTTON_THREE)
            c = sched.card(col)
            sched.answerCard(col, c!!, Consts.BUTTON_THREE)
            assertEquals(Counts(0, 2, 0), sched.counts(col))
            undo()
            reset()
            assertEquals(Counts(1, 1, 0), sched.counts(col))
            undo()
            reset()
            assertEquals(Counts(2, 0, 0), sched.counts(col))
