...
class DecksV16 :
    ...
    override fun id(col: Collection, name: str) = id(col, name, true, 0).get()

    @RustCleanup("only for java interface - should be removed")
    @RustCleanup("This needs major testing - the behavior had changed")
    override fun id_safe(col: Collection, name: String, @Suppress("UNUSED_PARAMETER") type: String) =
        id(col, name, create = true, type = 0).get()
    ...
    override fun all(col: Collection) =
        get_all_legacy(col).map { x -> Deck(x.getJsonObject()) }

    @Deprecated("decks.allIds() is deprecated, use .all_names_and_ids()")
    override fun allIds(col: Collection) =
        this.all_names_and_ids(col).map { x -> x.id }.toSet()
    ...
    override fun count(col: Collection) =
        len(this.all_names_and_ids(col))
    ...
    override fun confForDid(col: Collection, did: DeckId): DeckConfig {
        val deck = get(col, did, _default = false).toV16Optional()
    ...
    override fun updateConf(col: Collection, g: DeckConfig) =
        updateConf(col, DeckConfigV16.from(g), preserve_usn = false)
    ...
    fun for_card_ids(col: Collection, cids: List<Long>) =
        col.db.queryLongList("select did from cards where id in ${ids2str(cids)}")
    ...
    override fun selected(col: Collection) =
        col.backend.getCurrentDeck().id

    override fun current(col: Collection) =
        this.get(col, selected(col))
    ...
    override fun select(col: Collection, did: DeckId) {
        ...
        val active = deck_and_child_ids(col, did)
        if (active != active(col)) {
    ...
    override fun parents(col: Collection, did: DeckId) =
        parents(col, did, Optional.empty())
    ...
    fun parents(
        ...
        for (part in immediate_parent_path(get(col, did).toV16Optional().name)) {
        ...
        for (parent_name in parents_names) {
            ...
            if (nameMap.isPresent) {
                ...
                deck = get(col, this.id(col, parent_name))
            }
            parents.append(deck)
        }
        return parents
    ...
    override fun isDyn(col: Collection, did: DeckId): Boolean {
        return get(col, did).toV16().dyn != 0
    }
