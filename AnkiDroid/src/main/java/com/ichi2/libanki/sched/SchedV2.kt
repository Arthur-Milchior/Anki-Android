...
open class SchedV2(col: Collection) : AbstractSched() {
    ...
    protected var mDayCutoff: Long = 0
    final override fun dayCutoff(col: Collection) = mDayCutoff
    ...
    private fun deckDueList(col: Collection) = deckDueList(col, null)!!
    ...
    protected fun _fillNew(col: Collection) = _fillNew(col, false)
    ...
    private fun _fillNew(col: Collection, allowSibling: Boolean): Boolean {
        ...
        while (!mNewDids.isEmpty()) {
            ...
            val lim = min(mQueueLimit, _deckNewLimit(col, did, true))
            if (lim != 0) {
                mNewQueue.clear()
     ...
     * @param considerCurrentCard Whether current card should be counted if it is in this deck
     */
    protected fun _deckNewLimit(col: Collection, did: Long, fn: LimitMethod?, considerCurrentCard: Boolean): Int {
    ...
    private fun _rescheduleLrnCard(col: Collection, card: Card, conf: JSONObject, delay: Int? = null): Int {
    ...
    // Overridden
    protected open fun _rescheduleAsRev(col: Collection, card: Card, conf: JSONObject, early: Boolean) {
     ...
     * @param considerCurrentCard whether the current card should be taken from the limit (if it belongs to this deck)
     */
    protected open fun _deckRevLimitSingle(col: Collection, d: Deck?, considerCurrentCard: Boolean) = _deckRevLimitSingle(col, d, null, considerCurrentCard)
    ...
    protected fun _fillRev(col: Collection) = _fillRev(col, false)
    ...
    protected fun _rescheduleRev(col: Collection, card: Card, @BUTTON_TYPE ease: Int, early: Boolean) {
        // update interval
        card.lastIvl = card.ivl
        if (early) {
    ...
    protected fun _logRev(col: Collection, card: Card, @BUTTON_TYPE ease: Int, delay: Int, type: Int) {
        log(
            col,
            card.id,
            col.usn(),
    ...
    fun haveBuriedSiblings(col: Collection) = haveBuriedSiblings(col, col.decks.active(col))
    ...
    fun haveManuallyBuried(col: Collection) = haveManuallyBuried(col, col.decks.active(col))
  ...
  type
    ...
    override fun undoReview(col: Collection, card: Card, wasLeech: Boolean) {
        ...
        if (!wasLeech && card.note(col).hasTag(col, "leech")) {
