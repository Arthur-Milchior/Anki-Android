 ...
 * which show that note are generated. Some note types generate one card, others generate more than one.
 * Features:
...
class NoteEditor :
    ...
    fun hasUnsavedChanges(): Boolean {
        ...
        // changed note type?
        if (!addNote && currentEditedCard != null) {
            val newNoteType: NotetypeJson? = currentlySelectedNotetype
            val oldNoteType: NotetypeJson = currentEditedCard!!.noteType(getColUnsafe)
    ...
    private fun showCardTemplateEditor() {
        ...
        Timber.d(
            "showCardTemplateEditor() for note type %s",
    ...
    private fun updateCards(noteType: NotetypeJson?) {
        ...
        Timber.d("updateCards() template count is %s", tmpls.size)
        ...
        if (!addNote && tmpls.size < editorNote!!.notetype.tmpls.length()) {
    ...
    private fun allowFieldRemapping(): Boolean {
        // Map<String, Pair<Integer, NotetypeJson>> fMapNew = getCol().getNoteTypes().fieldMap(getCurrentlySelectedModel())
    ...
    private inner class EditNoteTypeListener : OnItemSelectedListener {
        override fun onItemSelected(
            ...
            if (allNoteTypeIds!![pos] != noteNoteTypeId) {
                ...
                val tmpls =
                    try {
                        newNoteType.getJSONArray("tmpls")
                    } catch (e: Exception) {
    ...
    @VisibleForTesting(otherwise = VisibleForTesting.NONE)
    fun setCurrentlySelectedNoteType(mid: NoteTypeId) {
        val position = allNoteTypeIds!!.indexOf(mid)
