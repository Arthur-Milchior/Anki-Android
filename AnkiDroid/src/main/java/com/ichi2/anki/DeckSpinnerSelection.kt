...
class DeckSpinnerSelection(
     ...
     */
    fun selectDeckById(col: Collection, deckId: DeckId, setAsCurrentDeck: Boolean): Boolean {
        return if (deckId == ALL_DECKS_ID) {
            selectAllDecks()
        } else {
            selectDeck(col, deckId, setAsCurrentDeck)
        }
    }
     ...
     */
    private fun selectDeck(col: Collection, deckId: DeckId, setAsCurrentDeck: Boolean): Boolean {
