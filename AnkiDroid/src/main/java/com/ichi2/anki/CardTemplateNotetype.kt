...
class CardTemplateNotetype(
    ...
    fun toBundle(): Bundle =
        bundleOf(
            INTENT_NOTE_TYPE_FILENAME to saveTempNoteType(AnkiDroidApp.instance.applicationContext, noteType),
    ...
    companion object {
        const val INTENT_NOTE_TYPE_FILENAME = "editedModelFilename"

        /**
         * Load the TemporaryNoteType from the filename included in a Bundle
         *
         * @param bundle a Bundle that should contain persisted JSON under INTENT_NOTE_TYPE_FILENAME key
        ...
        fun fromBundle(bundle: Bundle): CardTemplateNotetype? {
            val editedNoteTypeFileName = bundle.getString(INTENT_NOTE_TYPE_FILENAME)
            // Bundle.getString is @Nullable, so we have to check.
            if (editedNoteTypeFileName == null) {
                Timber.d("fromBundle() - note type file name under key %s", INTENT_NOTE_TYPE_FILENAME)
                return null
            }
